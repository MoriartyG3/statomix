...
#graph{min-width:1300px;background:white;border:1px solid #e2e8f0;border-radius:9px}.lane{fill:#f8fafc;stroke:#e2e8f0}.lane-title{font-size:14px;font-weight:700;fill:#0f172a}.stage-title{font-size:12px;font-weight:700;fill:#475569}
...
function render(){svg.innerHTML='';const defs=add('defs',{}),marker=add('marker',{id:'arrow',viewBox:'0 0 10 10',refX:9,refY:5,markerWidth:6,markerHeight:6,orient:'auto-start-reverse'},defs);add('path',{d:'M 0 0 L 10 5 L 0 10 z',fill:'#94a3b8'},marker);const nodes=visibleNodes(),ids=new Set(nodes.map(n=>n.node_id)),datasets=[...new Set(nodes.map(n=>n.dataset||'Project'))].sort(),positions={},lanes={};const laneHeader=42;let y=55;
for(const d of datasets){const dn=nodes.filter(n=>(n.dataset||'Project')===d),counts={};dn.forEach(n=>counts[stage[n.node_type]??2]=(counts[stage[n.node_type]??2]||0)+1);const h=Math.max(170,laneHeader+30+Math.max(1,...Object.values(counts))*88);lanes[d]=[y,h];for(let s=0;s<5;s++){dn.filter(n=>(stage[n.node_type]??2)===s).sort((a,b)=>a.node_id.localeCompare(b.node_id)).forEach((n,i)=>positions[n.node_id]=[170+s*270,y+laneHeader+28+i*88])}y+=h+18}
...
for(const d of datasets){const [top,h]=lanes[d];add('rect',{x:10,y:top,width:1380,height:h,rx:9,class:'lane'});const datasetNode=data.nodes.find(n=>(n.dataset||'Project')===d);const datasetLabel=datasetNode?.display_label||d;const t=add('text',{x:24,y:top+27,class:'lane-title'});t.textContent=datasetLabel}
