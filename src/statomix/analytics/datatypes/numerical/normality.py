...
class Normality:
    ...
    def __init__(
        ...
        n_total = len(series)
        self.series = pd.to_numeric(series.copy(), errors="raise").dropna()
        ...
        if ddof >= len(self.series):
            raise ValueError(
                f"ddof must be smaller than the non-missing sample size "
                f"({len(self.series)}), got {ddof}."
            )
        if not np.isfinite(self.series.to_numpy(dtype=float)).all():
            raise ValueError("Normality tests require finite observations.")

        self.alpha = alpha
        self.ddof = ddof
        self.mean = float(np.mean(self.series))
        self.std = float(np.std(a=self.series, ddof=self.ddof))
        if self.std == 0:
            raise ValueError(
                "Normality tests are undefined for a constant sample "
            ...
            "parametric_test": (
                (
                    ...
                    if n <= 5000
                        ...
                        "anderson" if not ties["ks_family_reliable"] else "lilliefors"
                    )
                ),
                ...
                "Above n=5000 this implementation uses Lilliefors, or "
                "Anderson-Darling when ties invalidate KS-family tests.",
            ),
            "symmetry_only": (
                "dagostino",
    ...
    def get_consensus(self, alpha=None):
        ...
        eligible = report[
            report["p_value_reliable"].fillna(False) & report["p"].notna()
        ]

        if eligible.empty:
            raise RuntimeError(
                "No normality tests produced a reliable p-value for consensus."
            )

        if alpha is not None:
            if not 0 < alpha < 1:
                raise ValueError(f"alpha must be in (0, 1), got {alpha!r}")
            votes = (eligible["p"] > alpha).sum()
        else:
            votes = eligible["normal"].sum()
    ...
    def get_distribution_shape(self):
        ...
        skew_msg += (
            " (right/positive)" if s > 0 else " (left/negative)" if s < 0 else ""
        )
    ...
    def _shapiro(self):
        ...
        with warnings.catch_warnings(record=True) as caught:
            ...
            p_value_reliable = not caught and np.isfinite(shapiro_p)
    ...
    def _ks(self):
        ...
        """
        standardized = (self.series - self.mean) / self.std
    ...
    def _dagostino(self):
        ...
        return {
            ...
            "p_value_reliable": (
                len(self.series) >= 8 and not caught and np.isfinite(dagostino_p)
            ),
        }
    ...
    def _jb(self):
        ...
        return {
            ...
            "p_value_reliable": (len(self.series) >= 20 and np.isfinite(jb_p)),
        }
    ...
    def _lilliefors(self):
        ...
        return {
            ...
            "recommended_sample_size": ">50",
            "p_value_reliable": self.get_tie_diagnostics()["ks_family_reliable"],
        }
    ...
    def _add_annotation(self, ax, test_type, loc, all_tests=False):
        ...
        text = (
            self._annotation_text_all()
            if all_tests
            else self._annotation_text(test_type)
        )
        ax.text(
            pos["x"],
            pos["y"],
    ...
    def plot_dashboard(
        ...
        if not show_all_tests and test_type not in self._implemented_tests:
            raise ValueError(f"test_type must be one of {self._implemented_tests}")
