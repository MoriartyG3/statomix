...
from statomix.analytics.datatypes.survival.data import (
    coerce_event_to_bool,
    prepare_survival_data,
)
from statomix.logging import get_logger

logger = get_logger(name="SingleClassSurv")
...
class SingleClassSurv:
    ...
    def __init__(self, surv_label: str, surv_df: pd.DataFrame):
        prepared = prepare_survival_data(frame=surv_df)
        if prepared.dropped_rows > 0:
            logger.warning(
                "[SingleClassSurv] Dropped %s row(s) with missing "
                "'time' or 'event' values.",
                prepared.dropped_rows,
            )

        self.kmf = KaplanMeierFitter()
        self.surv_label = surv_label
        self.surv_df = prepared.frame
    ...
    def _coerce_event_to_bool(event_series: pd.Series) -> pd.Series:
        ...
        """
        return coerce_event_to_bool(event=event_series)
    ...
    def _format_median(median_value: float, ci_lower: float, ci_upper: float) -> dict:
        ...
        """
        median_str = (
            "not reached" if not np.isfinite(median_value) else f"{median_value:.2f}"
        )
    ...
    def get_rmst(
        ...
        if bootstrap_ci:
            rmst_samples = []
            iterator = (
                tqdm(range(n_bootstraps)) if show_progress else range(n_bootstraps)
            )
            ...
            rmst_dict["bootstrap_label"] = (
                f"{mean_rmst:.2f} (95% CI, {ci[0]:.2f} - {ci[1]:.2f})"
            )
            rmst_dict["label"] = (
                f"{rmst_dict['rmst']:.2f} (95% CI, {ci[0]:.2f} - {ci[1]:.2f})"
            )
