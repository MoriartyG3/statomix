...
logger = get_logger(name="MultiClassSurv")
...
class MultiClassSurv:
    ...
    def __init__(
        ...
        grouping_cols = [c for c in surv_df_multi.columns if c not in ("time", "event")]
        ...
        if prepared.dropped_rows:
            logger.warning(
                "[MultiClassSurv] Dropped %s row(s) missing time, event, " "or group.",
        ...
        for idx, label in self.group_labels.items():
            mask = analysis_df[target_col_name] == label
            self.surv_dfs[idx] = analysis_df[mask][["time", "event"]].copy()
    ...
    def add_validation_to_analysis_config_file(path, max_row=500):
        ...
        add_datatype_list_validations(
            path=path,
            sheet_name=MultiClassSurv.MODULE_NAME,
            max_row=max_row,
        )
    ...
    def _resolve_baseline_group(
        self, baseline_group, categories, surv_df_multi
    ) -> tuple:
        ...
        ordered_labels = [baseline_label] + [
            c for c in categories if c != baseline_label
        ]
    ...
    def get_cox_ph_dict(self):
        ...
        if self.censoring == "right":
            self.cph.fit_right_censoring(
                cox_ph_df, duration_col="time", event_col="event"
            )
        else:
            raise NotImplementedError(
    ...
    def plot_km_curves(
        ...
        for idx in range(self.n_groups):
            ...
            kmf.plot_survival_function(
                ax=ax, label=f"{label}", ci_show=False, color=cmap(idx)
            )
        ...
        if add_risk_table:
            add_at_risk_counts(
                *kmfs_for_table, ax=ax, labels=labels_for_table, xticks=x_axis_range
            )
        ...
        plt.text(
            x=0.05,
            y=y,
            s=f"Overall {log_rank_dict['p_value_label']}",
